...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  ret.hid_to_class = ((class_prob - data.targets)*hid_output' ./ size(data.targets,2)) + wd_coefficient.*model.hid_to_class; #n_classes x n_hid
  ret.input_to_hid = (data.inputs*((((hid_output)).*(1 - hid_output)).*(((model.hid_to_class')*(class_prob - data.targets))))' ./ size(data.targets,2))' + wd_coefficient.*model.input_to_hid;
